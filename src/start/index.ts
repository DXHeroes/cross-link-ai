...
const findIntersections = async (
  ...
  for (let i = 0; i < myContent.length; i += batchSize) {
    const myBatch = myContent.slice(i, i + batchSize);
    const batchPromises = myBatch.flatMap((myItem) =>
      otherContent.map((otherItem) => async () => {
    ...
    intersections.push(...batchResults.flat());

    console.log(
      `Processed batch ${i / batchSize + 1} of ${Math.ceil(myContent.length / batchSize)}`,
    );
  }
